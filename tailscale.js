...
function main(config) {
  ...
  if (
    ...
    !globalGroup.proxies.includes(TAILSCALE_GROUP)
  ) {
    const selfIndex = groups.findIndex((g) => g && g.name === TAILSCALE_GROUP);
    const nextName = groups
      .slice(selfIndex + 1)
      .map((g) => g && g.name)
      .find((name) => name && name !== GLOBAL_GROUP && globalGroup.proxies.includes(name));
    if (nextName) {
      globalGroup.proxies.splice(globalGroup.proxies.indexOf(nextName), 0, TAILSCALE_GROUP);
    } else {
      globalGroup.proxies.push(TAILSCALE_GROUP);
    }
  }

...
function main(config) {
  ...
  if (
    ...
    !globalGroup.proxies.includes(HUGGINGFACE_GROUP)
  ) {
    const selfIndex = groups.findIndex((g) => g && g.name === HUGGINGFACE_GROUP);
    const nextName = groups
      .slice(selfIndex + 1)
      .map((g) => g && g.name)
      .find((name) => name && name !== GLOBAL_GROUP && globalGroup.proxies.includes(name));
    if (nextName) {
      globalGroup.proxies.splice(globalGroup.proxies.indexOf(nextName), 0, HUGGINGFACE_GROUP);
    } else {
      globalGroup.proxies.push(HUGGINGFACE_GROUP);
    }
  }
